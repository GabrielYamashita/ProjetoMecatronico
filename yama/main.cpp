...
DigitalOut Rele(PC_10);
Serial LCD(PA_9, PA_10);
...
//        estado = 2; // Vai para a Rotina de Posicionamento | IHM
    ...
    while(estado == 2) { // POSICIONAMENTO
        ...
        bool Transferencia = true; // processo dos soltas
        ...
        while(Transferencia) { // Checa se Foram Definidos os Pontos de Solta
            Transferencia = false; // Condição de Saída | IHM
        }
